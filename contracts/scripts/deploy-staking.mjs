 ...
 * Run: npx hardhat run scripts/deploy-staking.mjs --network localhost
 */
...
async function main() {
    ...
    const DragonStaking = await hre.ethers.getContractFactory("DragonStaking");
    ...
    const dragonToken = await hre.ethers.getContractAt("DragonToken", addresses.DragonToken);
    await (await dragonToken.setMinter(stakingAddress, true)).wait();
    console.log("   ✅ DragonStaking authorized to mint DGNE");

    // 3. Update GameConfig with staking contract address (if available)
    console.log("\n3️⃣ Configuring GameConfig...");
    const gameConfig = await hre.ethers.getContractAt("GameConfig", addresses.GameConfig);
